...
/*
 * This file is part of stacheport
 *
 *
...
<<<<<<< HEAD
    ...
    private function get_value($stache_match, $collection = null)
    {
        ...
        if (isset($collection)) {
        ...
        } elseif (isset( $this->data[$stache_match])) {
            ...
            if (stripos($stache_match, '.') !== false) { //dot notation for objects or array
                ...
                } else {                     //replace dot notation with obj notation
...
=======
        ...
        public $display=false,$data=array();

        public static function factory($template)
        {
            return new Stache($template);
        }
        ...
        private function run_loops($s)
        {
            ...
            $s = preg_replace('#\{\{\#(.+?)\}\}(.+?)\{\{\/(.+?)\}\}#is', $loop_value, $s, 1);
        ...
        private function process_template($template, $match = null, $collection = null)
        {
            ...
            if (strpos($stache_match, '{{') !== false) {
                $stache_match = substr($stache_match, strrpos($stache_match, '{{') + 2);
            }
            ...
            $template = preg_replace('+\{\{' . preg_quote($t1) . '\}\}+is', isset($value)?$value:'', $template, 1);
...
>>>>>>> 1707ff83abab6222163be8a6f1c7c3818ccf2c5b
